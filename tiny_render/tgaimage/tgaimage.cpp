    ...
    do
    {
        ...
        if (chunk_header < 128)
        {
            ...
            for (int i = 0; i < chunk_header; i++)
            {
                ...
                if (!in.good())
                {
                    std::cerr << "an error occured reading the header\n";
                    return false;
                }
...
bool TGAImage::write_tga_file(const char *filename, bool rle)
{
    ...
    header.image_descriptor = 0x20; //左上
...
bool TGAImage::unload_rle_data(std::ofstream &out)
{
    ...
    while (cur_pix < npixels)
    {
        unsigned long chunk_start = cur_pix * bytespp;
        unsigned long cur_byte = cur_pix * bytespp;
        unsigned char run_length = 1;
        bool raw = true;
        while (cur_pix + run_length < npixels && run_length < max_chunk_length)
        {
            bool succ_eq = true;
            for (int t = 0; succ_eq && t < bytespp; t++)
                succ_eq = (data[cur_byte + t] == data[cur_byte + t + bytespp]);
